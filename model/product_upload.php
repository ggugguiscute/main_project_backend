...
//1. 변수 정의 : 텍스트 변수, 이미지 파일 정보 변수, 날짜 변수, 이미지 확장자 필터링 변수, 파일 사이즈 제한 변수
...
//==2.
//확장자 필터링
if(!in_array($exp, $allowed_exp)){//in_array : 두 번째 파라미터 배열에 첫 번째 파라미터 문자열이 있으면 true
    echo json_encode(array("msg" => "허용되지 않은 이미지 파일 형식입니다."));
    exit();
}
...
$image_dir = $_SERVER['DOCUMENT_ROOT'].'/main_project/images/products/'; //사진이 저장되는 경로 지정
...
if($image_name && !$image_error){
    ...
    if(!move_uploaded_file($image_tmp_name, $upload_file)){
        echo json_encode(array("msg" => "사진이 업로드되지 않았습니다."));
        exit();}
} else {
    $image_name="";
}
