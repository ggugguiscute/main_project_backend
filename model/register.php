...
function signin($conn) {
    ...
    //==로그인로직
    //1. 받아온 아이디와 데이터베이스에서 존재하는 아이디 비교
    ...
    if(!mysqli_num_rows($result)){
        ...
        $login_data  = $result->fetch_array(); //회원 데이터 전체를 추출하여 저장

        $pwd_valid = password_verify($pwd, $login_data['user_pass']);
        //입력된 데이터(첫번째 파라미터)와 디비의 해시데이터(두번째 파라미터)를 비교하여 boolean값으로 반환(맞으면 true, 틀리면 false)
