...
function get_user($conn){
    ...
    if(!mysqli_num_rows($result)){
        ...
        array_push($json_result, array("user_idx" => $row['user_idx'],"user_name" => $row['user_name'],"user_id" => $row['user_id'],"user_email" => $row['user_email'],"user_lvl" => $row['user_lvl'])); //첫번째 파라미터 : 대상배열, 두번째 파라미터 : 배열 입력값
    }
    }
...
function patch_user($conn){
    ...
    // //업데이트 구문 : UPDATE [teble name] SET [update column] = [update value] WHERE [condition]
    ...
    if(!$stmt->prepare($sql)){
        ...
        echo json_encode(array("msg" => "레벨 수정에 실패하였습니다."));
    }

    $stmt -> bind_param("ss", $user_lvl, $user_idx); //sql 순서 같게
    $stmt -> execute();
